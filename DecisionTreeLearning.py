...
def DecisionTreeLearner(dataset, ce):
    ...
    def chooseAttribute(attributes, examples): #trova il piu importante attributo e relativa soglia in accordo all information gain (caso Entropy)
        maxgainAttr = 0
        thresholdAttr = 0
        ...
        for attr in attributes:
            ...
            for i in listValuesForAttribute[attr]:
                ...
                if gain > maxgainValue :                                          #scelgo il massimo gain
                    maxgainValue = gain
                    threshValue = float(i)
    ...
    def chooseAttribute2(attributes, examples): #trova il piu importante attributo e relativa soglia in accordo all information gain (caso classification error)
        maxgainAttr = 0
        thresholdAttr = 0
        listValuesForAttribute = getListValuesForAttribute(dataset.examples, dataset.target)  # prepara una lista di valori per ogni attributo per trovare il piu importante
        global mostImportanceA
        for attr in attributes:
            ...
            threshValue = 0
            for i in listValuesForAttribute[attr]:
                gain = float(informationGain2(attr, float(i), examples))     # calcola il suo gain
                if gain > maxgainValue :                                          #scelgo il massimo gain
                    maxgainValue = gain
                    threshValue = float(i)
            if maxgainValue  >= maxgainAttr:
    ...
    def informationGain(attribute, threshold, examples): #caso entropy
    ...
    def informationGain2(attribute, threshold, examples): #caso classification error
        def classification_error(examples): #definisco la funzione classification error
            maxp = 0
            if len(examples) != 0:
                for v in dataset.values:
                    p = float(counting(dataset.target, v, examples)) / len(examples)
        ...
        def remainder(examples):
            N = float(len(examples))
            ExampleMinor, ExampleMajor = splittingOnThreshold(attribute, threshold, examples)
            remainderExampleMinor = (float((len(ExampleMinor))) / N) * classification_error(ExampleMinor)
            remainderExampleMajor = (float((len(ExampleMajor))) / N) * classification_error(ExampleMajor)
            return (remainderExampleMinor + remainderExampleMajor)

        #formula che calcola l' information gain
        return classification_error(examples) - remainder(examples)
