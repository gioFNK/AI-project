...
import os
os.environ["PATH"] += os.pathsep + 'C:\Program Files (x86)\Graphviz2.38/bin' #da cambiare al momento dell'installazione
...
class DecisionTree:
    ...
    def GraphViz(rootnode, ce):
        print ('***************************************************************************')
        print ('Visita per livelli albero di decisione sul dataset per intero:')
        #utilizziamo gli IF clause per evitare lo stesso file name
        if(ce==0):#Caso Entropy
            u = Digraph('DecisionTree', filename='DecisionTreeEntropy.gv')
        else:#Caso Classification Error
            u = Digraph('DecisionTree', filename='DecisionTreeClassificationError.gv')
        u.attr(size='30,30')
